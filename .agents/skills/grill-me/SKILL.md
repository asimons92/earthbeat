...
name: grill-me
description: >-
  Interview the user one question at a time to resolve every open branch of a
  plan or decision before implementation. Use when the user says "grill me",
  "/grill-me", "interview me", "stress-test this plan", "poke holes", "push back
  on this", or asks for hard questions before committing to an approach. Also
  use proactively before plan mode when the user's request has unresolved design
  branches, ambiguous scope, or hidden assumptions. Do NOT use to test the
  user's knowledge of a subject. Do NOT use for code review or post-implementation
  critique. Adapted from Matt Pocock's grill-me skill.
---
...
For non-engineering subjects, substitute relevant branches: tradeoffs, downside scenarios, exit conditions, who pays the cost, what evidence would change the answer.
...
- **Probe until specific.** Vague answers get followed up until concrete.
...
- **Surface hidden assumptions.** Name assumptions that a later decision depends on.
...
- **Suggested next step** — usually `/plan`, occasionally more research on a named topic.
...
- Do not pad with affirmations. Ask the next question.
...
Plain prose. No code blocks for the questions themselves. Number the questions as you go so the user can refer back. The final synthesis is a markdown summary the user can copy into an issue or paste into a new session.

## Source

- Matt Pocock, [grill-me skill](https://www.aihero.dev/my-grill-me-skill-has-gone-viral)
- Upstream: [mattpocock/skills/grill-me](https://github.com/mattpocock/skills/blob/main/grill-me/SKILL.md)
